...
class DigitRecognizerGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Multi-Layer Perceptron - Digit Recognizer")
    ...
    def setup_ui(self):
        ...
        # Initial status
        self.update_status(f"Status: {self.status_text}\n\n")
        self.update_status("Network Architecture:\n")
        self.update_status("  Input Layer:  784 neurons\n")
        self.update_status("  Hidden Layer: 128 neurons\n")
        self.update_status("  Output Layer: 10 neurons\n\n")
        self.update_status("Ready to train or test!\n")
    ...
    def start_training(self):
        ...
        self.train_button.config(state=tk.DISABLED, bg='#cccccc')
    ...
    def train_network(self):
        ...
        def training_callback(sample, total, accuracy, loss, image, label):
            ...
            if sample % 50 == 0 or sample == total:  # Update every 50 samples
                ...
                status = f"Sample: {sample}/{total}\n"
                status += f"True Label: {label}\n"
                status += f"Accuracy: {accuracy:.2f}%\n"
        ...
        self.is_training = False
        self.train_button.config(state=tk.NORMAL, bg='#4CAF50')
    ...
    def upload_image(self):
        ...
        try:
            ...
            # Display results
            self.update_status("\n" + "="*40 + "\n", clear=True)
            self.update_status("PREDICTION RESULTS\n")
            self.update_status("="*40 + "\n\n")
            self.update_status(f"Predicted Digit: {predicted}\n")
            self.update_status(f"Confidence: {confidence*100:.2f}%\n\n")
            self.update_status("All probabilities:\n")
            
            for digit in range(10):
                prob = output[digit] * 100
                bar = "█" * int(prob / 5)
                self.update_status(f"  {digit}: {prob:5.2f}% {bar}\n")
